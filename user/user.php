...
<body>
    ...
    $user_count = $mysqlClient->prepare("SELECT count(id) as user_count FROM user WHERE firstname LIKE CONCAT('%', :nametext, '%') OR lastname LIKE CONCAT('%', :nametext, '%')");
    $user_count->execute([
    ...
    if (isset($getelementpage)) {
        ...
        if (isset($_GET['ide'])) {
            ...
            $nbr_element_page = 10;
            header("Location: user.php?page=1&ide=$nbr_element_page&title=$gettitleMovie");
        }
    }
            ...
            <tbody>
                ...
                <?php for ($i = 0; $i < count($users); $i++) : ?>
                    <tr>
                        ...
                        <td class="modifier"><a href="user_read.php?id=<?php echo ($users[$i]['id']); ?>">🖋️</a></td>
                        <td class="supprimer"><a href="user_delete.php?id=<?php echo ($users[$i]['id']); ?>">🗑️</a></td>
                    </tr>
                <?php endfor; ?>
